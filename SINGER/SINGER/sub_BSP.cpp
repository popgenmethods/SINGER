...
void sub_BSP::start(set<Branch> &start_branches, set<Interval_info> &start_intervals, float t) {
    cut_time = t;
    curr_index = 0;
...
void sub_BSP::set_cutoff(float x) {
    cutoff = x;
}
...
void sub_BSP::set_check_points(set<float> &p) {
    check_points = p;
}

void sub_BSP::forward(float rho) {
    if (branch_change) {
        update(rho);
    } else {
        regular_forward(rho);
...
void sub_BSP::regular_forward(float rho) {
...
void sub_BSP::null_emit(float theta, Node_ptr query_node) {
...
void sub_BSP::mut_emit(float theta, float bin_size, set<float> &mut_set, Node_ptr query_node) {
...
map<float, Branch> sub_BSP::sample_joining_branches(int start_index, vector<float> &coordinates) {
    prev_rho = -1;
...
void sub_BSP::compute_recomb_probs(float rho) {
    if (prev_rho == rho) {
        return;
    }
    for (int i = 0; i < dim; i++) {
...
void sub_BSP::compute_null_emit_prob(float theta, Node_ptr query_node) {
...
void sub_BSP::compute_mut_emit_probs(float theta, float bin_size, set<float> &mut_set, Node_ptr query_node) {
    for (int i = 0; i < dim; i++) {
        mut_emit_probs[i] = eh->mut_emit(curr_intervals[i]->branch, join_times[i], theta, bin_size, mut_set, query_node);
    }
}

void sub_BSP::transfer_helper(Interval_info &next_interval, Interval_ptr &prev_interval, float w) {
...
vector<float> &sub_BSP::get_join_times(int x) {
    auto time_it = all_join_times.upper_bound(x);
    time_it--;
    return time_it->second;
}

vector<float> &sub_BSP::get_join_weights(int x) {
...
void sub_BSP::simplify(map<float, Branch> &joining_branches) {
