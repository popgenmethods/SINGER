...
float RSP::exact_sample_start_time(Branch b, int density, float join_time, float cut_time) {
...
pair<Branch, float> RSP::exact_sample_start_time(Branch b1, Branch b2, int density, float join_time, float cut_time) {
...
void RSP::exact_sample_recombination(Recombination &r, float cut_time, Tree tree) {
    if (r.pos == 0) {
        return;
    }
    if (r.start_time > 0) {
...
float RSP::recomb_pdf(float s, float t) {
    assert(s <= t);
    return exp(s-t);
}

float RSP::exact_recomb_pdf(float s, float t) {
    float pdf = 1.0;
    float curr_time = s;
...
void RSP::get_coalescence_rate(Tree tree, Recombination r, float cut_time) {
    coalescence_rates.clear();
    set<float> coalescence_times = {numeric_limits<float>::infinity()};
    vector<float> sorted_coalescence_times = {};
    for (Branch b : tree.branches) {
        coalescence_times.insert(max(cut_time, b.lower_node->time));
    }
    coalescence_times.erase(r.deleted_node->time);
    sorted_coalescence_times = vector<float>(coalescence_times.begin(), coalescence_times.end());
    int n = (int) sorted_coalescence_times.size();
    for (int i = 0; i < n; i++) {
        coalescence_rates.insert({sorted_coalescence_times[i], n-i-1});
    }
}
