...
void TSP_smc::set_check_points(set<double> &p) {
    check_points = p;
}
...
void TSP_smc::start(Branch &branch, double t) {
    cut_time = t;
    curr_index = 0;
    curr_branch = branch;
...
double TSP_smc::get_exp_quantile(double p) {
    assert(p >= 0 and p <= 1);
    if (p < 1e-6) {
        return 0;
    }
...
vector<double> TSP_smc::generate_grid(double lb, double ub) {
    assert(lb < ub);
...
double TSP_smc::recomb_cdf(double s, double t) {
    if (isinf(t)) {
        return 1;
    }
    if (t == 0) {
...
double TSP_smc::recomb_quantile(double s, double q, double lb, double ub) {
    double mid_q = 0;
    double mid = lb;
    double p = 0;
    double w = 0;
    double gap = min(0.5*(ub - lb), 1e-5);
    while (ub - mid > gap) {
        p = recomb_cdf(s, mid);
        if (p >= q) {
            lb = lb;
...
void TSP_smc::forward(double rho) {
...
void TSP_smc::null_emit(double theta, Node_ptr query_node) {
...
void TSP_smc::mut_emit(double theta, double bin_size, set<double> &mut_set, Node_ptr query_node) {
...
map<double, Node_ptr > TSP_smc::sample_joining_nodes(int start_index, vector<double> &coordinates) {
    prev_rho = -1;
...
double TSP_smc::recomb_prob(double s, double t1, double t2) {
    assert(t1 <= t2);
...
double TSP_smc::psmc_cdf(double rho, double s, double t) {
    double l;
    double pre_factor;
    if (t <= s) {
...
double TSP_smc::standard_recomb_cdf(double rho, double s, double t) {
    double l = 2*s;
    double integral;
    double cdf;
    if (t <= s) {
...
double TSP_smc::psmc_prob(double rho, double s, double t1, double t2) {
...
void TSP_smc::generate_intervals(Branch &next_branch, double lb, double ub) {
...
double TSP_smc::random() {
    double p = uniform_random();
    return p;
}

double TSP_smc::get_prop(double lb1, double ub1, double lb2, double ub2) {
    double p;
    if (ub2 - lb2 < 1e-6) {
        p = 1;
    } else {
...
void TSP_smc::compute_null_emit_probs(double theta, Node_ptr query_node) {
...
void TSP_smc::compute_mut_emit_probs(double theta, double bin_size, set<double> &mut_set, Node_ptr query_node) {
...
void TSP_smc::compute_diagonals(double rho) {
    if (rho == prev_rho) {
        return;
    }
    double t;
...
void TSP_smc::compute_lower_diagonals(double rho) {
    if (rho == prev_rho) {
        return;
    }
    double t;
...
void TSP_smc::compute_upper_diagonals(double rho) {
    if (rho == prev_rho) {
        return;
    }
    upper_diagonals[0] = 0;
...
void TSP_smc::compute_emissions(set<double> &mut_set, Branch branch, Node_ptr node) {
...
void TSP_smc::compute_trace_back_probs(double rho, Interval *interval, vector<Interval *> &intervals) {
    if (rho == prev_rho) {
        return;
    }
...
double TSP_smc::sample_time(double lb, double ub) {
    return exp_median(lb, ub);
}

double TSP_smc::sample_time(double lb, double ub, double t) {
    double new_t = 0;
    double p = uniform_random();
    double lq = 1 - exp(-lb);
    double uq = 1 - exp(-ub);
    double q = p*lq + (1 - p)*uq;
    double x = -log(1 - q);
    if (ub < 0.01) {
...
double TSP_smc::exp_median(double lb, double ub) {
    assert(lb <= ub);
    if (isinf(ub)) {
        return lb + 2*random();
    }
